    ...
    return (
        <>
            <div className="space-y-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div className="relative w-full md:w-auto flex-1 max-w-md hidden md:block">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 border border-blue-500/50 rounded-lg focus:outline-none focus:border-yellow-400 bg-[#000c24]/80 text-white placeholder-blue-300 transition-colors shadow-inner"
                        />
                    </div>

                    <div className="flex gap-2 w-full md:w-auto justify-end">
                        <label className="cursor-pointer bg-blue-900/50 border border-blue-500/50 hover:bg-blue-800 text-yellow-400 px-4 py-2 rounded-lg flex items-center gap-2 shadow-sm transition-all font-bold">
                            <Upload size={18} />
                        ...
                        <button
                            ...
                            className="bg-blue-900/50 border border-blue-500/50 hover:bg-blue-800 text-blue-300 p-2 rounded-lg shadow-sm transition-all"
                        ...
                        <button
                            ...
                            className="bg-yellow-500 text-blue-950 px-4 py-2 rounded-lg hover:bg-yellow-400 shadow-[0_0_15px_rgba(255,215,0,0.3)] flex items-center gap-2 font-bold transition-transform active:scale-95"
                        >
                            <Plus size={18} />
                ...
                {/* Tabela de questões */}
                <div className="overflow-hidden rounded-xl border border-blue-500/30 shadow-[0_0_15px_rgba(0,0,0,0.3)] bg-[#000c24]/60 backdrop-blur-sm">
                    {loading && (
                        <div className="p-8 text-center text-blue-300 flex flex-col items-center font-bold">
                            <div className="w-8 h-8 border-2 border-yellow-500 border-t-transparent rounded-full animate-spin mb-2"></div>
                            Carregando questões...
                        </div>
                    )}
                    {error && <p className="p-8 text-center text-red-400 font-bold">{error}</p>}
                                        ...
                                        questions.map((question) => (
                                            <tr key={question._id} className="hover:bg-blue-900/40 transition-colors group">
                                                ...
                                                <td className="py-4 px-6">
                                                    <span className="bg-blue-800 text-blue-200 border border-blue-600 px-2 py-1 rounded text-xs font-bold uppercase">{question.tema}</span>
                                                </td>
                                                <td className="py-4 px-6">
                                                    <span className={`px-2 py-1 rounded text-xs font-bold uppercase border
                                                        ${question.dificuldade === 'facil' ? 'bg-green-900/50 text-green-400 border-green-500/50' :
                                                            question.dificuldade === 'medio' ? 'bg-yellow-900/50 text-yellow-400 border-yellow-500/50' :
                                                ...
                                                <td className="py-4 px-6 text-right">
                                                    <div className="flex justify-end gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                                                        <button
                                                            ...
                                                            className="p-2 text-blue-300 hover:bg-blue-800 hover:text-white rounded-lg transition-colors border border-transparent hover:border-blue-500/50"
                                                            title="Editar"
                                                        >
                                                            <Edit2 size={18} />
                                                        </button>
